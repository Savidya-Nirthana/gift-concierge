...
class KaprukaCrawler:
    def __init__(self, base_url: str, max_depth: int, exclude_patterns: List[str], max_pages: int = 500, save_steps: int = 20):
    ...
    async def crawl_async(self, start_urls: List[str], request_delay: float = 2.0) -> List[Dict[str, Any]]:
        queue = deque([(self._normalize_url(url), 0) for url in start_urls])
        queued_urls: Set[str] = set(item[0] for item in queue)  # fast lookup instead of list scan
            ...
            while queue:
                ...
                try:
                    ...
                    if self._is_product_page(soup):
                        # Product page — extract full content + links
                        ...
                        if len(doc_data["content"]) > 100:
                            ...
                            print(f"✅ [{depth}] Product page added ({len(self.documents)} total)")
                        else:
                            print(f"⚠️ [{depth}] Product page skipped (content too short): {url}")
                    else:
                        # Category/listing page — extract links only
                        doc_data = self.extract_category_links(soup, url)
                        doc_data["url"] = url
                        doc_data["depth_level"] = depth
                        print(f"📂 [{depth}] Category page — extracted {len(doc_data['links'])} links: {url}")
